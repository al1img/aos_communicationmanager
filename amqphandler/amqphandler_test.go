...
func TestConnectionEvents(t *testing.T) {
	...
	amqpHandler.SubscribeForConnectionEvents(connectionConsumer)
...
func TestSendDisconnectMessages(t *testing.T) {
	...
	// Send number important messages equals to send queue size - should be accepted without error
	...
	// Next important message should fail due to send queue size

	if err := amqpHandler.SendInstanceStateRequest(
		cloudprotocol.StateRequest{}); !errors.Is(err, amqphandler.ErrSendQueueFull) {
		t.Errorf("Wrong error type: %v", err)
	}
